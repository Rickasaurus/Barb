module Barb.Tests.PredicateLanguageOOOTests
...
[<Fact>]
let ``should preserve left-to-right order of operations with parens`` () = 
...
[<Fact>]
let ``should preserve left-to-right order of operations with record bool`` () = 
    let testRecord = { HasHat = true; Name = "Howard" }
    let predicate = buildExpr<BoolRec,bool> "HasHat and Name = \"Howard\""
    let result = predicate testRecord
    Assert.True(result)    
...
[<Fact>]
let ``should preserve left-to-right order of operations with explicit bool`` () = 
...
[<Fact>]
let ``should evalute order of boolean ops correctly`` () = 
...
[<Fact>]
let ``should use correct order of operations`` () = 
...
[<Fact>]
let ``should use correct order of operations with getters 1`` () = 
    let predicate = buildExpr<NumRec,bool> "Num + 4 / 4 = 3"
    let result = predicate { Num = 2 }
    Assert.True(result)    

[<Fact>]
let ``should use correct order of operations with getters 2`` () = 
    let predicate = buildExpr<NumRec,bool> "2 + Num / 4 = 3"
    let result = predicate { Num = 4 }
    Assert.True(result)

[<Fact>]
let ``should use correct order of operations with getters 3`` () = 
